-- | doctest utilities
module Flat.Instances.Test(tst,tstBits,asList,module Data.Word) where

import Flat.Class(Flat(..))
import Flat.Run(flat,unflat)
import Flat.Bits(bits,asBytes)
...
tstBits v = (unflat (flat v) == Right v,Flat.Class.size v 0,prettyShow . bits $ v) 
