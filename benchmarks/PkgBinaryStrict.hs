{-# LANGUAGE PackageImports ,NoMonomorphismRestriction ,FlexibleInstances ,MultiParamTypeClasses#-}
module PkgBinaryStrict(PkgBinaryStrict(..),serlN2) where

import Types
import Test.Data
import BinaryDerive
import Data.Binary.Strict.Get
import Data.Binary.Strict.Put
import Data.Binary.Strict as Binary
import qualified Data.ByteString.Lazy as BS

data PkgBinaryStrict a = PkgBinaryStrict {unPkg::a} deriving (Eq,Show)

instance Arbitrary a => Arbitrary (PkgBinaryStrict a) where arbitrary = fmap PkgBinaryStrict arbitrary

{-
instance Binary.Binary a => Serialize (PkgBinaryStrict a) where
  serialize (PkgBinaryStrict a) = Binary.encode $ a
  deserialize = either (Left . error . show) (\(_,_,v) -> Right $ PkgBinaryStrict v) . Binary.decodeOrFail
-}

instance Binary a => Serialize PkgBinaryStrict a where
  serialize (PkgBinaryStrict a) = Binary.encode a
  deserialize = either (Left . error . show)  (\(_,_,v) -> Right $ PkgBinaryStrict v) . Binary.decodeOrFail
  pkg = PkgBinaryStrict
  unpkg (PkgBinaryStrict a) = a

-- Tests
tt = ser $ PkgBinaryStrict tree1
...
z = (\v-> let Right (PkgBinaryStrict a) = deserialize PkgBinaryStrict.serlN2 in a == v) lN2
